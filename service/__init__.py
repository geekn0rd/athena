...
def eyes(frame, results, color):
    landmarks = np.round(results[0]).astype(int)
    # Draw right eye (points 36-41)
    for p in landmarks[36:42]:
        cv2.circle(frame, tuple(p), 2, color, -1, cv2.LINE_AA)
    cv2.polylines(frame, [landmarks[36:42]], True, color, thickness=1, lineType=cv2.LINE_AA)
    
    # Draw left eye (points 42-47)
    for p in landmarks[42:48]:
        cv2.circle(frame, tuple(p), 2, color, -1, cv2.LINE_AA)
    cv2.polylines(frame, [landmarks[42:48]], True, color, thickness=1, lineType=cv2.LINE_AA)
